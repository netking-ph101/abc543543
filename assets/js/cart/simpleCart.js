	...
	var typeof_string = typeof "",
		...
		generateSimpleCart = function (space) {
			...
			var selectorEngines = {
				...
				// default options
				settings = {
					checkout: { type: "PayPal", email: "you@yours.comy" },
					currency: "TWD",
					...
					, {
						...
						, callback: function () {
							...
							$button.closest("." + namespace + "_shelfItem").descendants().each(function (x, item) {
								...
								// 處理日期欄位
								var $dateField = simpleCart.$('.item_date'); // 假設這是日期欄位的 class
								var dateVal = $dateField.val(); // 獲取日期值
								...
								if (/(boat|car)-\d+\.html/i.test(window.location.pathname)) {
									...
									// 將日期值轉換為 ISO 格式字串
									var isoDate1 = new Date(dateVal1).toISOString().split('T')[0];

									fields['session'] = isoDate1; // 將日期值添加到 fields 物件中
								}
